...
@Secured(value = [SecurityRules.IS_AUTHENTICATED])
 class LuckPlatformController(
  private val luckPlatformService: LuckPlatformService
    ...
    @SingleResult
	@Secured(value = [SecurityRules.IS_AUTHENTICATED])
     fun create(@Valid @Body luckPlatformDTO: LuckPlatformDTO): Mono<Rsp<LuckPlatformVO>> {
    ...
    @SingleResult
	@Secured(value = [SecurityRules.IS_AUTHENTICATED])
     fun createList(@Valid @Body luckPlatformDTO: List<LuckPlatformDTO>): Mono<Rsp<List<LuckPlatformVO?>>> {
        val param=LuckPlatformMapper.MAPPER.dto2bo(luckPlatformDTO)
	...
	*/
	...
	@Secured(value = [SecurityRules.IS_AUTHENTICATED])
     fun update(
			    ...
			    @Parameter(description = "唯一ID",example = "1541693333435453441",required =  true)
				id:Long,
				@Valid @Body luckPlatformPageDTO: LuckPlatformPageDTO): Mono<Rsp<LuckPlatformVO?>> {
	...
	@Secured(value = [SecurityRules.IS_AUTHENTICATED])
     fun getRecord(
			    ...
			    @QueryValue("id")
			    @Parameter(description = "唯一ID",example = "1541693333435453441",required =  true)
				id:Long):Mono<Rsp<LuckPlatformVO?>> {
	...
	@Secured(value = [SecurityRules.IS_AUTHENTICATED])
     fun getPageList(@QueryValue pageable: PageableDTO,@QueryValue pageDTO:LuckPlatformPageDTO):Mono<Rsp<RspPagination<LuckPlatformVO?>>> {
	...
	*/
	...
	@Secured(value = [SecurityRules.IS_ADMIN])
     fun getPageLogicList(@QueryValue pageable:PageableDTO,@QueryValue  pageDTO:LuckPlatformPageDTO):Mono<Rsp<RspPagination<LuckPlatformVO?>>>  {
        val param=LuckPlatformMapper.MAPPER.pageDto2bo(pageDTO)
	...
	*/
	...
	@Secured(value = [SecurityRules.IS_ADMIN])
     fun  getList(@QueryValue pageDTO:LuckPlatformPageDTO):Mono<Rsp<List<LuckPlatformVO?>>> {
       val param=LuckPlatformMapper.MAPPER.pageDto2bo(pageDTO)
	...
	*/
    ...
    @Get("/all")
	@Secured(value = [SecurityRules.IS_ADMIN])
     fun  getAll():Mono<Rsp<List<LuckPlatformVO?>>> {
	...
	*/
    ...
    @Get("/all/unLimit")
	@Secured(value = [SecurityRules.IS_ADMIN])
     fun getAllLogicList(@Nullable authentication:Authentication):Mono<Rsp<List<LuckPlatformVO?>> > {

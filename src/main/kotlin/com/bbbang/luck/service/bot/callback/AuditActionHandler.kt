...
class AuditActionHandler() {
    ...
    @Inject
    lateinit var luckCreditApplyService: LuckCreditApplyService


     fun pass(absSender: DefaultAbsSender, callbackQuery: CallbackQuery){
    ...
    fun reject(absSender: DefaultAbsSender, callbackQuery: CallbackQuery){
        val fromId = callbackQuery.from.id
        luckUserService.findByBotUser(callbackQuery).flatMap {
            luckWalletService.findWalletByUserId(it.id,it.groupId)
        }
